      ...
      .select({
        ...
        isLiked: exists(
          db
            ...
            .from(likes)
            .where(
              and(eq(likes.profilesId, userId), eq(likes.replyId, reply.id))
            )
        ).as("isLiked"),
        isRePosted: exists(
          db
...
export async function POST(req: Request) {
  ...
  if (idFromPost) {
    ...
    return NextResponse.json({ success: true, message: "Reply of post is created", res });
  } else if (idFromReply) {
    const res = await db
      .insert(reply)
      .values({
    ...
    return NextResponse.json({ success: true, message: "Reply of reply is created", res });
  }
}
